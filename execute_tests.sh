...
parentpath="/home/guest/jsoup_copy"
srcpath="/home/guest/jsoup_copy/src/main/java"
cpath="/home/guest/jsoup_copy/target/classes"
...
do
	...
	for fileName in `ls -1 ./target/surefire-reports/*.xml`
	  do
		...
		curl -X POST $url/api/external/import/ -H 'content-type: multipart/form-data; boundary=----WebKitFormBoundary7MA4YWxkTrZu0gW' -H "token: $apiKey" -F "file=@$fileName" -F 'project_name'=$project -F 'test_suite_name'=$testsuite -F 'type'=$importtype -F 'commit'=$commitId
	  done
